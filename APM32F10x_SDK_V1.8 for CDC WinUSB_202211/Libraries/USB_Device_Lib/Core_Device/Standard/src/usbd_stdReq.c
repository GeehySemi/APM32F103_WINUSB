...
#if USB_SUP_WINSUB
static uint8_t USBD_WinUSBGetDescriptor(void);
#endif

#if USB_SUP_WINSUB
/*!
 * @brief       USB request winUSB request
 *
 * @param       None
 *
 * @retval      None
 */
void USBD_WinUSBReqeust(void)
{
    uint8_t result = 1;
    ...
    switch (bRequest)
    {
        case USB_REQ_MS_VENDOR_CODE:
            result = USBD_WinUSBGetDescriptor();
            break;
...
static uint8_t USBD_WinUSBGetDescriptor(void)
{
    uint8_t ret = SUCCESS;
    uint32_t len = 0;
    uint8_t wIndex0 = g_usbDev.reqData.byte.wIndex[0];
    uint8_t wIndex1 = g_usbDev.reqData.byte.wIndex[1];
    
    switch(wIndex0)
    {
        case 0x04:
            len = USB_MIN(*(uint16_t*)g_usbDev.reqData.byte.wLength, g_usbDev.pWinUSBOsFeatureDesc->size);
            USBD_CtrlInData((uint8_t*)g_usbDev.pWinUSBOsFeatureDesc->pDesc, len);
            break;
        
        case 0x05:
            len = USB_MIN(*(uint16_t*)g_usbDev.reqData.byte.wLength, g_usbDev.pWinUSBOsPropertyDesc->size);
            USBD_CtrlInData((uint8_t*)g_usbDev.pWinUSBOsPropertyDesc->pDesc, len);
            break;
...
static uint8_t USBD_StandardGetDescriptor(void)
{
    ...
    else if (wValue1 == USBD_DESC_STRING)
    {
            ...
            {
                len = USB_MIN(*(uint16_t*)g_usbDev.reqData.byte.wLength, g_usbDev.pStringDesc[4].size);
                USBD_CtrlInData((uint8_t*)g_usbDev.pStringDesc[4].pDesc, len);
            }
#else
                len = USB_MIN(*(uint16_t*)g_usbDev.reqData.byte.wLength, g_usbDev.pStringDesc[wValue0].size);
                USBD_CtrlInData((uint8_t*)g_usbDev.pStringDesc[wValue0].pDesc, len);
